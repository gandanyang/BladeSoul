...
try {
    ...
    if ($needsImport) {
        Write-Host '--- 0/28 import new assets ---' -ForegroundColor Yellow
        & $godot --headless --path $root --import | Out-Null
    }

    Write-Host '--- 1/28 build ---' -ForegroundColor Cyan
    dotnet build
    if ($LASTEXITCODE -ne 0) { throw "build failed (exit $LASTEXITCODE)" }

    Write-Host '--- 2/28 unit tests (pure logic) ---' -ForegroundColor Cyan
    dotnet test tests\Oniblade.Tests\Oniblade.Tests.csproj
    if ($LASTEXITCODE -ne 0) { throw "unit tests failed (exit $LASTEXITCODE)" }

    Write-Host '--- 3/28 resource self-test (headless engine) ---' -ForegroundColor Cyan
    if (-not (Test-Path -LiteralPath $godot)) {
        ...
        Write-Host '--- 4/28 combat smoke test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 5/28 deflect training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 6/28 guard-spam contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 7/28 dodge training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 8/28 dodge-too-early contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 9/28 perilous baseline: normal slash (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 10/28 perilous contrast: threat thrust (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 11/28 heal mid-drink test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 12/28 battle reset test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 13/28 issen: in window (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 14/28 issen: safe window (防劝退核心) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 15/28 issen: too early (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 16/28 issen: too late (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 17/28 actor id uniqueness (headless engine) ---' -ForegroundColor Cyan
        ...
        # 而不是代码里写死的 1。
        Write-Host '--- 18/28 revive: samurai (1 revive) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 19/28 revive: migoto (3 revives) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 20/28 combat vfx (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 21/28 companion dialogue (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 22/28 whitebox: dojo (walkability + clearance) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 23/28 whitebox: gifu chapter 1 ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 24/28 atmosphere: rain/fog/lanterns (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 25/28 atmosphere benchmark smoke (headless, 数字不可信) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 26/28 combat hud (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 27/28 stance closeout (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 28/28 indoor light (headless engine) ---' -ForegroundColor Cyan
