...
try {
    ...
    if ($needsImport) {
        Write-Host '--- 0/31 import new assets ---' -ForegroundColor Yellow
        & $godot --headless --path $root --import | Out-Null
    }

    Write-Host '--- 1/31 build ---' -ForegroundColor Cyan
    dotnet build
    if ($LASTEXITCODE -ne 0) { throw "build failed (exit $LASTEXITCODE)" }

    Write-Host '--- 2/31 unit tests (pure logic) ---' -ForegroundColor Cyan
    dotnet test tests\Oniblade.Tests\Oniblade.Tests.csproj
    if ($LASTEXITCODE -ne 0) { throw "unit tests failed (exit $LASTEXITCODE)" }

    Write-Host '--- 3/31 resource self-test (headless engine) ---' -ForegroundColor Cyan
    if (-not (Test-Path -LiteralPath $godot)) {
        ...
        Write-Host '--- 4/31 combat smoke test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 5/31 deflect training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 6/31 guard-spam contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 7/31 dodge training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 8/31 dodge-too-early contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 9/31 perilous baseline: normal slash (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 10/31 perilous contrast: threat thrust (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 11/31 heal mid-drink test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 12/31 battle reset test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 13/31 issen: in window (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 14/31 issen: safe window (防劝退核心) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 15/31 issen: too early (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 16/31 issen: too late (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 17/31 actor id uniqueness (headless engine) ---' -ForegroundColor Cyan
        ...
        # 而不是代码里写死的 1。
        Write-Host '--- 18/31 revive: samurai (1 revive) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 19/31 revive: migoto (3 revives) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 20/31 combat vfx (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 21/31 companion dialogue (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 22/31 whitebox: dojo (walkability + clearance) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 23/31 whitebox: gifu chapter 1 ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 24/31 atmosphere: rain/fog/lanterns (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 25/31 atmosphere benchmark smoke (headless, 数字不可信) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 26/31 combat hud (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 27/31 stance closeout (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 28/31 indoor light (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 29/31 fall recovery + animation coverage gaps (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 30/31 charged attack (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 31/31 jump (headless engine) ---' -ForegroundColor Cyan
