...
try {
    ...
    if ($needsImport) {
        Write-Host '--- 0/33 import new assets ---' -ForegroundColor Yellow
        & $godot --headless --path $root --import | Out-Null
    }

    Write-Host '--- 1/33 build ---' -ForegroundColor Cyan
    dotnet build
    if ($LASTEXITCODE -ne 0) { throw "build failed (exit $LASTEXITCODE)" }

    Write-Host '--- 2/33 unit tests (pure logic) ---' -ForegroundColor Cyan
    ...
    Write-Host '--- 3/33 resource self-test (headless engine) ---' -ForegroundColor Cyan
    if (-not (Test-Path -LiteralPath $godot)) {
        ...
        Write-Host '--- 4/33 combat smoke test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 5/33 deflect training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 6/33 guard-spam contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 7/33 dodge training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 8/33 dodge-too-early contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 9/33 perilous baseline: normal slash (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 10/33 perilous contrast: threat thrust (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 11/33 heal mid-drink test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 12/33 battle reset test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 13/33 issen: in window (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 14/33 issen: safe window (防劝退核心) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 15/33 issen: too early (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 16/33 issen: too late (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 17/33 actor id uniqueness (headless engine) ---' -ForegroundColor Cyan
        ...
        # 而不是代码里写死的 1。
        Write-Host '--- 18/33 revive: samurai (1 revive) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 19/33 revive: migoto (3 revives) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 20/33 combat vfx (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 21/33 companion dialogue (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 22/33 whitebox: dojo (walkability + clearance) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 23/33 whitebox: gifu chapter 1 ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 24/33 atmosphere: rain/fog/lanterns (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 25/33 atmosphere benchmark smoke (headless, 数字不可信) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 26/33 combat hud (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 27/33 stance closeout (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 28/33 indoor light (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 29/33 fall recovery + animation coverage gaps (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 30/33 charged attack (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 31/33 jump (headless engine) ---' -ForegroundColor Cyan
        ...
        # 只看它会让清场永远等不到）。
        Write-Host '--- 32/33 encounter (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 33/33 save point (headless engine) ---' -ForegroundColor Cyan
