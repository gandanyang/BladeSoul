...
try {
    ...
    if ($needsImport) {
        Write-Host '--- 0/26 import new assets ---' -ForegroundColor Yellow
        & $godot --headless --path $root --import | Out-Null
    }

    Write-Host '--- 1/26 build ---' -ForegroundColor Cyan
    dotnet build
    if ($LASTEXITCODE -ne 0) { throw "build failed (exit $LASTEXITCODE)" }

    Write-Host '--- 2/26 unit tests (pure logic) ---' -ForegroundColor Cyan
    dotnet test tests\Oniblade.Tests\Oniblade.Tests.csproj
    if ($LASTEXITCODE -ne 0) { throw "unit tests failed (exit $LASTEXITCODE)" }

    Write-Host '--- 3/26 resource self-test (headless engine) ---' -ForegroundColor Cyan
    if (-not (Test-Path -LiteralPath $godot)) {
        ...
        Write-Host '--- 4/26 combat smoke test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 5/26 deflect training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 6/26 guard-spam contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 7/26 dodge training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 8/26 dodge-too-early contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 9/26 perilous baseline: normal slash (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 10/26 perilous contrast: threat thrust (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 11/26 heal mid-drink test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 12/26 battle reset test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 13/26 issen: in window (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 14/26 issen: safe window (防劝退核心) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 15/26 issen: too early (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 16/26 issen: too late (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 17/26 actor id uniqueness (headless engine) ---' -ForegroundColor Cyan
        ...
        # 而不是代码里写死的 1。
        Write-Host '--- 18/26 revive: samurai (1 revive) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 19/26 revive: migoto (3 revives) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 20/26 combat vfx (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 21/26 companion dialogue (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 22/26 whitebox: dojo (walkability + clearance) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 23/26 whitebox: gifu chapter 1 ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 24/26 atmosphere: rain/fog/lanterns (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 25/26 atmosphere benchmark smoke (headless, 数字不可信) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 26/26 combat hud (headless engine) ---' -ForegroundColor Cyan
