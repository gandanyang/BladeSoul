...
try {
    ...
    if ($needsImport) {
        Write-Host '--- 0/25 import new assets ---' -ForegroundColor Yellow
        & $godot --headless --path $root --import | Out-Null
    }

    Write-Host '--- 1/25 build ---' -ForegroundColor Cyan
    dotnet build
    if ($LASTEXITCODE -ne 0) { throw "build failed (exit $LASTEXITCODE)" }

    Write-Host '--- 2/25 unit tests (pure logic) ---' -ForegroundColor Cyan
    dotnet test tests\Oniblade.Tests\Oniblade.Tests.csproj
    if ($LASTEXITCODE -ne 0) { throw "unit tests failed (exit $LASTEXITCODE)" }

    Write-Host '--- 3/25 resource self-test (headless engine) ---' -ForegroundColor Cyan
    if (-not (Test-Path -LiteralPath $godot)) {
        ...
        Write-Host '--- 4/25 combat smoke test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 5/25 deflect training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 6/25 guard-spam contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 7/25 dodge training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 8/25 dodge-too-early contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 9/25 perilous baseline: normal slash (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 10/25 perilous contrast: threat thrust (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 11/25 heal mid-drink test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 12/25 battle reset test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 13/25 issen: in window (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 14/25 issen: safe window (防劝退核心) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 15/25 issen: too early (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 16/25 issen: too late (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 17/25 actor id uniqueness (headless engine) ---' -ForegroundColor Cyan
        ...
        # 而不是代码里写死的 1。
        Write-Host '--- 18/25 revive: samurai (1 revive) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 19/25 revive: migoto (3 revives) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 20/25 combat vfx (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 21/25 companion dialogue (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 22/25 whitebox: dojo (walkability + clearance) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 23/25 whitebox: gifu chapter 1 ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 24/25 atmosphere: rain/fog/lanterns (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 25/25 atmosphere benchmark smoke (headless, 数字不可信) ---' -ForegroundColor Cyan
