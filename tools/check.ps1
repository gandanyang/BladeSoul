...
try {
    ...
    if ($needsImport) {
        Write-Host '--- 0/34 import new assets ---' -ForegroundColor Yellow
        & $godot --headless --path $root --import | Out-Null
    }

    Write-Host '--- 1/34 build ---' -ForegroundColor Cyan
    dotnet build
    if ($LASTEXITCODE -ne 0) { throw "build failed (exit $LASTEXITCODE)" }

    Write-Host '--- 2/34 unit tests (pure logic) ---' -ForegroundColor Cyan
    ...
    Write-Host '--- 3/34 resource self-test (headless engine) ---' -ForegroundColor Cyan
    if (-not (Test-Path -LiteralPath $godot)) {
        ...
        Write-Host '--- 4/34 combat smoke test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 5/34 deflect training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 6/34 guard-spam contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 7/34 dodge training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 8/34 dodge-too-early contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 9/34 perilous baseline: normal slash (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 10/34 perilous contrast: threat thrust (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 11/34 heal mid-drink test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 12/34 battle reset test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 13/34 issen: in window (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 14/34 issen: safe window (防劝退核心) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 15/34 issen: too early (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 16/34 issen: too late (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 17/34 actor id uniqueness (headless engine) ---' -ForegroundColor Cyan
        ...
        # 而不是代码里写死的 1。
        Write-Host '--- 18/34 revive: samurai (1 revive) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 19/34 revive: migoto (3 revives) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 20/34 combat vfx (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 21/34 companion dialogue (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 22/34 whitebox: dojo (walkability + clearance) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 23/34 whitebox: gifu chapter 1 ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 24/34 atmosphere: rain/fog/lanterns (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 25/34 atmosphere benchmark smoke (headless, 数字不可信) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 26/34 combat hud (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 27/34 stance closeout (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 28/34 indoor light (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 29/34 fall recovery + animation coverage gaps (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 30/34 charged attack (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 31/34 jump (headless engine) ---' -ForegroundColor Cyan
        ...
        # 只看它会让清场永远等不到）。
        Write-Host '--- 32/34 encounter (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 33/34 save point (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 34/34 tutorial act 1 (headless engine) ---' -ForegroundColor Cyan
