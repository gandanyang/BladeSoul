...
try {
    ...
    if ($needsImport) {
        Write-Host '--- 0/32 import new assets ---' -ForegroundColor Yellow
        & $godot --headless --path $root --import | Out-Null
    }

    Write-Host '--- 1/32 build ---' -ForegroundColor Cyan
    dotnet build
    if ($LASTEXITCODE -ne 0) { throw "build failed (exit $LASTEXITCODE)" }

    Write-Host '--- 2/32 unit tests (pure logic) ---' -ForegroundColor Cyan
    ...
    if (-not (Test-Path -LiteralPath $godot)) {
        ...
        Write-Host '--- 4/32 combat smoke test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 5/32 deflect training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 6/32 guard-spam contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 7/32 dodge training test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 8/32 dodge-too-early contrast (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 9/32 perilous baseline: normal slash (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 10/32 perilous contrast: threat thrust (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 11/32 heal mid-drink test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 12/32 battle reset test (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 13/32 issen: in window (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 14/32 issen: safe window (防劝退核心) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 15/32 issen: too early (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 16/32 issen: too late (whiff) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 17/32 actor id uniqueness (headless engine) ---' -ForegroundColor Cyan
        ...
        # 而不是代码里写死的 1。
        Write-Host '--- 18/32 revive: samurai (1 revive) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 19/32 revive: migoto (3 revives) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 20/32 combat vfx (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 21/32 companion dialogue (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 22/32 whitebox: dojo (walkability + clearance) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 23/32 whitebox: gifu chapter 1 ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 24/32 atmosphere: rain/fog/lanterns (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 25/32 atmosphere benchmark smoke (headless, 数字不可信) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 26/32 combat hud (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 27/32 stance closeout (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 28/32 indoor light (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 29/32 fall recovery + animation coverage gaps (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 30/32 charged attack (headless engine) ---' -ForegroundColor Cyan
        ...
        Write-Host '--- 31/32 jump (headless engine) ---' -ForegroundColor Cyan
        ...
        # 只看它会让清场永远等不到）。
        Write-Host '--- 32/32 encounter (headless engine) ---' -ForegroundColor Cyan
