...
| T25 | 主角模型 | 执行 agent | 🟡 **部分**：资产已登记，**未接入游戏**（无骨架） | T24 |
| **T31** | **战斗 HUD 与反馈层**（血槽 / 架势槽 / 弹开一闪提示 / 伤害数字 / 交互提示） | 🟢 **可领取** | ⚪ 未开始 | 见 docs/11 |
...
| T29 | 同伴「绫」模型（6 个动作，**无战斗动画**） | 🟡 依赖 T24 流程 | ⚪ 未开始 | 09 §10 / 03 §2.7 |
...
**规格全文**：[09 §10](09-主角外观设定.md) + 人设 [03 §2.7](03-剧情与关卡流程.md)。

**一句话**：**她比主角亮。** 丛云是全深色，她整体明度高一档——
这是"她一笑，世界就稍微亮一点"的视觉化。

### 交付物

- 模型（20~25 岁，162cm，旧白 + 苔绿 + 铜）
- 腰间三件：**小药囊 / 铜铃 / 旧木牌**（后两者是剧情线索，要做清楚）
- **6 个动作**：`idle` / `walk` / `sit` / `hand_item` / `treat` / `talk`
- 材质槽与主角同分组命名（统一光照）
...
1. `tools\check.ps1` 12 步全绿。
2. **同框截图**：她 + 丛云 + 1 个魔骸站在一起，**三个人一眼可分**（人 / 人 / 怪）。
3. 三角面 ≤ 20k。
4. **不要求战斗动画**——她从不进入战斗区域（03 §2.7 的机制保障）。
