...
| **T31** | **战斗 HUD 与反馈层**（血槽 / 架势槽 / 弹开一闪提示 / 伤害数字 / 交互提示） | 🔴 **最高优先**（试玩后提升） | ⚪ 未开始 | 见 docs/11 |
| **T37** | **架势系统收口**（死配置 + 玩家破防无表现） | 🟢 **可领取**（小） | ⚪ 未开始 | — |
| **T38** | **动画覆盖补齐**（现在只有 6~7 个，07 §1.1 要 ~28 个） | 🟢 可领取 | ⚪ 未开始 | T24 流程 |
| **T39** | **重出 3D 生成用的参考图与提示词**（旧图两次生成都不满意） | 🔴 **阻塞 T27/T35/T36** | ⚪ 未开始 | 见 12 §6 |
| **T32** | **关卡白盒 + 网格规格冻结**（场景资产的**前置**，必须先做） | 🔴 **最高优先** | ⚪ 未开始 | 见 10 §3.4 |
| T33 | 场景模块化套件（地面/墙/门洞/灯笼/町屋/神社） | 🟡 依赖 T32 的网格规格 | ⚪ 未开始 | T32 |
| T34 | 氛围与灯光（雨 / 体积雾 / 灯笼暖光点） | 🟢 可领取（**可并行**） | ⚪ 未开始 | 10 §3.1 |
| **T35** | **跑通本地 3D 生成通道**（Hunyuan3D-2，Maker 的免费后备） | 🔴 **建议尽早** | ⚪ 未开始 | 见 docs/12 |
...
**来源**：2026-09-13 试玩反馈 —— **"角色动画太少，很多缺的"**。

**现状**（`src/Player/HumanoidAnimator.cs`）：只有 **6~7 个**
——`idle / 走 / 跑` + 攻击 + 格挡姿势 + 受击 + 一闪。
