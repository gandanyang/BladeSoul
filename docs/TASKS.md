...
| **T37** | **架势系统收口**（死配置 + 玩家破防无表现） | 🟢 **可领取**（小） | ⚪ 未开始 | — |
...
## T37 · 架势系统收口（两个真实缺口，小卡）

**来源**：2026-09-13 试玩反馈。**机制层已经在跑**，但有两个洞：
...
> ⚠️ **这已经是第三次同类问题**：`PerfectDodgeGraceFrames`（物理上不成立）→
> `ReviveCount`（未实现）→ 现在这个。**"定义了但没人读"是本项目最高频的 bug 类型。**

**做法**：把难度档的值写进玩家 `Posture.RegenScale`（照 `GuardState` 那套
"切入前由输入侧写入"的用法）。
...
结果：破防时只有状态机进了 50 帧硬直，**画面和声音上什么都没发生**——
玩家只会觉得"我卡住了"，而不知道为什么。

**做法**：
- 玩家覆写 `OnPostureBroken()`：一个明显的姿态 + 音效（`CombatSfx.GuardBreak` 已存在）
