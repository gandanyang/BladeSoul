...
**状态**：①②③ **全部已修复并验证**（2026-09-15 深夜，制作人拍板「继续」）；
③ 的演出时长 60 帧是**临时默认值**，要调就改 `ActorStats.DeathPerformanceFrames`（`data/actors/enemy_grunt.tres`）。
...
### ③ 死亡演出接线 —— ✅ 已修（60 帧为临时默认，数据可调）

制作人拍板「继续」后接线。**两处都动了**：

1. `CombatActor._PhysicsProcess`：IsDead 分支不再裸 `return`，改调新虚钩子
   `TickDeathVisual(dt)`（基类空实现 = 没演出的角色行为不变）。
   **没有复用 `OnTickVisual`**——那里有冷却递减、转朝向等活人才该有的副作用，让死人走它会一边倒地一边转圈。
2. `Ashigaru` 覆写 `TickDeathVisual`：帧号自己累积（`_deathFrame`），播 `AshigaruAction.Death`；
   播完 t 停在 1，尸体停在最终倒地姿势。`DriveModel` 里那段永远走不到、写死 `0,0` 的死代码已删。

**总帧数的家**：`ActorStats.DeathPerformanceFrames`（与 `PostureBrokenFrames` 同一模式——
敌人自己的属性，不挂场景）。`data/actors/enemy_grunt.tres` 显式写 **60**；
姿势曲线里倒地占前 75% ⇒ 45 帧倒完、60 帧完全静止。**60 是临时默认值**，要调就改数据。

**验证**（探针现跑）：`Die()` 后 40 帧姿态差 **60.88°**（修前 0.00°；对照组满幅 61.0°）；
带窗截图确认尸体前扑倒地（`t52_death_after40.png`）。
...
```powershell
# 数值（无头，约 10 秒）。**修复后退出码 0**；若再报 1 = 回归了
